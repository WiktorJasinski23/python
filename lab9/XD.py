...
for i in range(white_keys):
    tkl.Button(nextframe, bg = 'white', command=lambda i=i: clicked('White', i)).grid(row=0, column=i*3, rowspan=2, columnspan=3, sticky='nsew')

for i in range(white_keys - 1):
    if black[i]:
        tkl.Button(nextframe, command=lambda i=i: clicked('Black', i)).grid(row=0, column=(i*3)+2, rowspan=1, columnspan=2, sticky='nsew')

for i in range(white_keys * 3):
    nextframe.columnconfigure(i, weight=1)

for i in range(2):
    nextframe.rowconfigure(i, weight=1)
